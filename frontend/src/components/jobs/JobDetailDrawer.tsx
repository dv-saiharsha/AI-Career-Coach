...
export function JobDetailDrawer({
  ...
  /* The drawer stays mounted while it slides out, and by then `job` may
     already be null — so the last non-null one is held and rendered
      ...
      {(state) =>
        ...
        <>
          <div
            data-state={state}
            onClick={onClose}
            className="presence-scrim fixed inset-0 z-40 bg-(--color-ink)/25 backdrop-blur-sm"
            aria-hidden="true"
          />
          ...
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby="shown-drawer-title"
            data-state={state}
            className="presence-drawer fixed right-0 top-0 z-50 flex h-full w-full max-w-xl flex-col border-l border-(--color-canvas-line) bg-(--color-canvas-raise) shadow-(--shadow-pop)"
          >
            <div className="flex items-start justify-between gap-4 border-b border-(--color-canvas-line) p-5">
              <div className="flex min-w-0 items-start gap-3">
                ...
                <div className="min-w-0">
                  <h2
                    id="shown-drawer-title"
                    className="text-base font-semibold leading-snug text-(--color-ink)"
                  >
                    {shown.title}
                  </h2>
                  <p className="mt-1 text-sm text-(--color-ink-dim)">{shown.company}</p>
                </div>
              </div>
              <button
                ref={closeRef}
                onClick={onClose}
                aria-label="Close shown details"
            ...
            <div className="flex-1 overflow-y-auto p-5">
              <div className="flex flex-wrap items-center gap-2 text-xs text-(--color-ink-dim)">
                ...
                <span className="inline-flex items-center gap-1">
                  ...
                  {postedLabel(shown.postedDaysAgo)}
                </span>
              </div>
              ...
              {shown.match && (
                <section className="mt-4 rounded-xl border border-(--color-canvas-line) bg-(--color-canvas-deep) p-4">
                  <div className="flex items-center gap-4">
                    {shown.match.overallMatch != null && shown.match.band ? (
                      <ScoreRing value={shown.match.overallMatch} band={shown.match.band} label="Resume Match" size={84} strokeWidth={6} />
                    ) : (
                      <div className="text-xs text-(--color-ink-faint)">
                        Not enough information in this listing to score a match.
                      </div>
                    )}
                    <p className="text-sm leading-relaxed text-(--color-ink-subtle)">{shown.match.explanation}</p>
                  </div>

                  {shown.match.skillsMatch && (
                  ...
                  {shown.match.skillsMatch && shown.match.skillsMatch.learningRecommendations.length > 0 && (
                    <div className="mt-4 flex items-start gap-2 border-t border-(--color-canvas-line) pt-4">
                      ...
                      <p className="text-xs leading-relaxed text-(--color-ink-dim)">
                        Worth learning: {shown.match.skillsMatch.learningRecommendations.join(' · ')}
                      </p>
                    </div>
                  )}
                </section>
              ...
              {shown.skills.length > 0 && !shown.match?.skillsMatch && (
            ...
            <div className="space-y-2 border-t border-(--color-canvas-line) p-5">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <button
                  onClick={() => onPracticeInterview(shown)}
                  className="btn-secondary flex items-center justify-center gap-2"
                >
              ...
              {onSaveToPipeline && (
                <button
                  onClick={() => onSaveToPipeline(shown)}
              ...
              {/* Straight to the split view rather than through the analyzer:
                  ...
                  posting wants. */}
              <Link
                href={`/cover-letter?job=${shown.id}`}
